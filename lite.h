...
struct Option {
	...
	const char *description;      /* What does the option do? */
	unsigned int expects;         /* How many arguments are expected. */
	char *type;                   /* The type of something */
	_Bool required;               /* True or false */
	union opt_value {
		char *str; 
		union file_value {
			FILE *file; 
		...
		int num; 
		char nil;
	} value;                      /* A long option for help? */
	int (*validator)(void *item); /* Validator function to check values */
	void (*filter)(void *item);   /* A function to filter or modify the value */
	char set;                     /* True or false */
};
#endif
...
int32_t memtok (const void *a, const uint8_t *tokens, int32_t rng, int32_t tlen);
...
#ifndef NOOPTIONS
 /*Options*/
 //OBJECT(opt) * NEW(opt)(int argc, char **argv, opt_t *lopt);/*These run both ways*/
 void opt_usage(Option *opts, const char *msg, int status);	
 //void die(Option *opts, unsigned int code, const char *msg);	
 //void opt_dump(Option *opts);	
 void opt_eval (Option *opts, int argc, char **argv, _Bool dump);
 //void free(struct OBJECT(opt) *opt);	
 union opt_value *get(Option *opts, const char *name);	
 unsigned int isset(Option *opts, const char *name);	
 #define opt_set(n) opt->isset(opt->opts, n)
 #define opt_getn(n) (!get(opt->opts, n) ? 0 : get(opt->opts, n)->num)
 #define opt_gets(n) (!get(opt->opts, n) ? NULL : get(opt->opts, n)->str)
 #define opt_getf(n) (!get(opt->opts, n) ? NULL : get(opt->opts, n)->file_value)
 #define opt_getfn(n) (!get(opt->opts, n) ? NULL : get(opt->opts, n)->file_value.fpath)
 #define opt_zgetn(n, o) \
   (!opt_getn(n) ? o : opt_getn(n))
 #define opt_zgets(n, o) \
   (!opt_gets(n) ? o : opt_gets(n))
#endif
#endif
 ...
 //void printf(struct LIST *self);
 unsigned int populate(struct LIST *self);
 unsigned int append(struct LIST *self, void *item);
 unsigned int prepend(struct LIST *self, void *item);
 unsigned int remove(struct LIST *self, void *item);
 unsigned int find(struct LIST *self);
 unsigned int size(struct LIST *self);
 listnode_t * next(struct LIST *self);
#endif
#ifndef NOFILES
 /*Files*/
