...
TODO:
	...
	- buffer tests need to be rewritten
	- list tests need to be rewritten
	- generic free and print if possible
	- nalloc?
	- change __name functions to __prefix_name
 * ------------------------------------------- */
#include "lite.h"
...
void socket_open (Socket *sock) {
	...
	if (!sk) {
		...
		if (!(sock = (Socket *)nalloc(sizeof(socket_t), "socket.socket")))
		{
			free(s);
			return NULL;
		}
	...
	sock->fd = socket(sock->domain, sock->conntype, sock->protocol);
	sock->opened = 1;
...
_Bool socket_accept (Socket *sock, Socket *new)
{
	...
	/* Accept a connection. */	
	new->fd = accept(sock->fd, NULL, NULL);

	/* If failure occurred, free all and send back. */
	if (!new->fd || new->fd == -1)
	{
		char *err = (new->fd == -1)?strerror(errno):"Lost descriptor.";
...
/*An option evaluation loop - dump should be a debug option*/
void opt_eval (Option *opts, int argc, char **argv, _Bool dump) {
	/*OPTION MACROS*/
	/*Tests work the same way as in canvas*/
	#define opt(sht,lng) \
